	...
	public List<CountrySortModel> search(String str, List<CountrySortModel> list)
	{
		...
		{// 正则表达式 匹配以数字或者加号开头的字符串(包括了带空格及-分割的号码)
			...
			for (CountrySortModel contact : list)
			{
				...
				{
					if (contact.simpleCountryNumber.contains(simpleStr) || contact.countryName.contains(str))
					{
		...
		} else
		{
			for (CountrySortModel contact : list)
			{
				if (contact.countryNumber != null && contact.countryName != null)
				{
					...
					if (contact.countryName.toLowerCase(Locale.CHINESE).contains(str.toLowerCase(Locale.CHINESE))
							|| contact.countrySortKey.toLowerCase(Locale.CHINESE).replace(" ", "").contains(str.toLowerCase(Locale.CHINESE))
